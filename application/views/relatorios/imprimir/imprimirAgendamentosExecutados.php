                        ...
                        <table width="100%" class="table table-bordered" style="border:1px solid #ccc; border-collapse:collapse;">
                            <thead>
                                <tr>
                                    ...
                                    <th style="font-size: 13px">Prof. Executante</th>
                                    <th style="font-size: 13px">It. do Agendamento</th>
                                    <th style="font-size: 13px">Prof. Saúde Exec.</th>
                                    <th style="font-size: 13px">Unidade Executante</th>
                                    <th style="font-size: 13px">Data Exec.</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php $total = 0; ?>
                                <?php foreach ($agendamentos as $a) : ?>
                                    <?php if (!empty($a->status_da_solicitacao) && $a->status_da_solicitacao == 'AUTORIZADO') { $total++; ?>
                                    <tr>
                                        <td><?= $a->idAgendamento ?></td>
                                        <td><?= $a->nome_cidadao ?? '' ?></td>
                                        <td><?= $a->cpf_cidadao ?? '' ?></td>
                                        <td><?= $a->nome_prof_executante ?? '' ?></td>
                                        <td><?= $a->nome_procedimento ?? '' ?> - <?= $a->nome_local ?? '' ?></td>
                                        <td><?= $a->nome_prof_exec_saude ?? '' ?></td>
                                        <td><?= $a->unidade_executante ?? '' ?></td>
                                        <td><?= isset($a->data_executante) ? date('d/m/Y', strtotime($a->data_executante)) : '' ?></td>
                                    </tr>
                                    <?php } ?>
                                <?php endforeach ?>
                            </tbody>
