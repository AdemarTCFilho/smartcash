...
class ContasPagar extends MY_Controller
{
    ...
    public function exportarExcel()
    {
        ...
        $writer->writeSheetHeader('Contas a Pagar', $cabecalho);
        foreach ($dados as $d) {
            $writer->writeSheetRow('Contas a Pagar', [
                $d->nomeCliente,
                $d->nomeUsuario,
                $d->nomeEmpresa,
                $d->nomeUnidade,
        ...
        force_download('contas_a_pagar_' . date('d-m-Y') . '.xlsx', $arquivo);
    }
    ...
    public function exportarPdf()
    {
        ...
        $mpdf->SetHTMLFooter('<table width="100%" style="font-size:9px; color:#666;"><tr>'
            ...
            . '</tr></table>');
        $mpdf->WriteHTML($html);
        $mpdf->Output(FCPATH . 'assets/uploads/temp/ultimo_export_pagar.pdf', 'F');

        header('Cache-Control: no-cache, no-store, must-revalidate');
        header('Pragma: no-cache');
        header('Expires: 0');
        $mpdf->Output('contas_a_pagar_' . date('d-m-Y') . '_' . time() . '.pdf', 'I');
    }
    ...
    {
        $dados = $this->contasPagar_model->getAllContasPagar();
