...
// Inclui a biblioteca XLSXWriter para geração de relatórios XLSX
...
class Relatorios extends MY_Controller
{
    ...
    public function cidadaos()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de cidadãos.');
            redirect(base_url());
        }
        $this->data['view'] = 'relatorios/rel_cidadaos';
        return $this->layout();
    }
    ...
    // Não somar 1 dia ao dataFinal, para garantir que o filtro <= dataFinal inclua o próprio dia
    ...
    public function cidadaosCustom()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de cidadãos.');
            redirect(base_url());
        ...
        $data['cidadaos'] = $this->Relatorios_model->cidadaosCustom($dataInicial, $dataFinal, $this->input->get());
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Cidadãos Customizado';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirCidadaos', $data, true);
        pdf_create($html, 'relatorio_clientes' . date('d/m/y'), true);
    }

    public function cidadaosRapid()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de cidadãos.');
            redirect(base_url());
        ...
        if ($format == 'xls') {
            $cidadaos = $this->Relatorios_model->cidadaosRapid($array = true);
            $cabecalho = [
                'Código' => 'integer',
                'Nome' => 'string',
                'Telefone' => 'string',
                'data_nasc' => 'MM-DD-YYYY',
                'Sexo' => 'string',
                'CNS' => 'string',
                'CPF' => 'string',
                'Nome da Mãe' => 'string',
            ...
            foreach ($cidadaos as $cidadao) {
                $writer->writeSheetRow('Sheet1', $cidadao);
            }

            $arquivo = $writer->writeToString();
            $this->load->helper('download');
            force_download('relatorio_cidadaos.xlsx', $arquivo);
            return;
        }

        $data['cidadaos'] = $this->Relatorios_model->cidadaosRapid();
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Cidadãos';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);

        $this->load->helper('mpdf');

        $html = $this->load->view('relatorios/imprimir/imprimirCidadaos', $data, true);
        pdf_create($html, 'relatorio_cidadaos' . date('d/m/y'), true);
    }
    ...
    public function AgendamentosRapid()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar relatórios de agendamentos.');
            redirect(base_url());
        }
        ...
        if ($format == 'xls') {
            // Geração XLSX (exemplo básico, pode ser expandido conforme padrão do sistema)
            $this->load->library('XLSXWriter');
            $dados = $this->Relatorios_model->AgendamentosRapid(true);
            $writer = new XLSXWriter();
            $header = array_keys($dados[0] ?? []);
            $writer->writeSheetHeader('Agendamentos', array_fill_keys($header, 'string'));
            foreach ($dados as $row) {
                $writer->writeSheetRow('Agendamentos', array_values($row));
            }
            $filename = 'relatorio_agendamentos_' . date('d-m-Y') . '.xlsx';
        ...
        $data['agendamentos'] = $this->Relatorios_model->AgendamentosRapid();
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Agendamentos';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirAgendamentos', $data, true);
        pdf_create($html, 'relatorio_agendamentos_' . date('d-m-Y'), true);
    }

    public function relRelatorioAnalitico()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar relatório analítico.');
            redirect(base_url());
        }

        $this->data['unidades_solicitantes'] = $this->agendamentos_model->get('unidade_solicitante', 'idUnidadeSolicitante, nome', '', 0, 0, false);
        
        $this->data['view'] = 'relatorios/rel_relatorio_analitico';
        return $this->layout();
    }
    ...
    public function agendamentos()
    {
        ...
        $this->data['view'] = 'relatorios/rel_agendamentos';
        return $this->layout();
    }

    // Novo método para relatório de agendamentos solicitados
    public function agendamentosSolicitados()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar relatórios de agendamentos.');
            redirect(base_url());
        }
        $this->data['view'] = 'relatorios/rel_agendamentos_solicitados';
        return $this->layout();
    }

    // Novo método para relatório de agendamentos executados
    public function agendamentosExecutados()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar relatórios de agendamentos.');
            redirect(base_url());
        }
        $this->data['view'] = 'relatorios/rel_agendamentos_executados';
        return $this->layout();
    }

    public function AgendamentosCustom()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar relatórios de agendamentos.');
            redirect(base_url());
        ...
        $dataInicial = $this->input->get('dataInicial');
        $dataFinal = $this->input->get('dataFinal');
        $status = $this->input->get('status_da_solicitacao');
        $format = $this->input->get('format');

        // Montar array de filtros para o relatório
        $where = [];
        if ($this->input->get('cidadaos_id')) {
            $where['cidadaos_id'] = $this->input->get('cidadaos_id');
        } elseif ($this->input->get('nomeCidadao')) {
            $where['pesquisa'] = $this->input->get('nomeCidadao');
        }
        if ($dataInicial) {
            $where['data'] = $dataInicial;
        }
        if ($dataFinal) {
            $where['data2'] = $dataFinal;
        }
        if ($status) {
            $where['status_da_solicitacao'] = $status;
        }

        $isXls = $format === 'xls';
        $agendamentos = $this->Relatorios_model->AgendamentosComFiltro($where);
        ...
        if ($isXls) {
            $cabecalho = [
                ...
                'CPF' => 'string',
                'Usuário' => 'string',
                'Prof. Solicitante' => 'string',
                'Prof. Executante' => 'string',
                'Prof. Saúde' => 'string',
                'Procedimento' => 'string',
                'Local' => 'string',
                'Unidade Solicitante' => 'string',
                'Unidade Executante' => 'string',
            ];
            $writer = new XLSXWriter();
            $writer->writeSheetHeader('Agendamentos', $cabecalho);
            foreach ($agendamentos as $row) {
                $writer->writeSheetRow('Agendamentos', is_object($row) ? (array)$row : $row);
            }
            $filename = 'relatorio_agendamentos_custom_' . date('d-m-Y') . '.xlsx';
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment; filename="' . $filename . '"');
            header('Cache-Control: max-age=0');
            $writer->writeToStdOut();
        ...
        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirAgendamentos', $data, true);
        pdf_create($html, 'relatorio_agendamentos_custom_' . date('d-m-Y'), true);
    }

    public function AgendamentosSolicitadosCustom()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar relatórios de agendamentos.');
            redirect(base_url());
        ...
        $data['agendamentos'] = $agendamentos;
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Agendamentos Solicitados';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);
        $data['dataInicial'] = $dataInicial;
        $data['dataFinal'] = $dataFinal;

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirAgendamentosSolicitados', $data, true);
        pdf_create($html, 'relatorio_agendamentos_custom_' . date('d-m-Y'), true);
    }

    public function AgendamentosExecutadosCustom()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar relatórios de agendamentos.');
            redirect(base_url());
        ...
        $data['agendamentos'] = $agendamentos;
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Agendamentos Executados';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);
        $data['dataInicial'] = $dataInicial;
        $data['dataFinal'] = $dataFinal;

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirAgendamentosExecutados', $data, true);
        pdf_create($html, 'relatorio_agendamentos_custom_' . date('d-m-Y'), true);
    }
    ...
    public function receitasBrutasCustom()
    {
        ...
        /**
     * Relatório Analítico de Agendamentos
     * Recebe filtros via GET e monta dados analíticos agrupados conforme o tipo de análise.
     */
    public function RelatorioAnalitico()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vCidadao')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios analíticos.');
            redirect(base_url());
        }

        $tipo_analise = $this->input->get('tipo_analise');
        $dataInicial = $this->input->get('dataInicial');
        $dataFinal = $this->input->get('dataFinal');
        $nomeCidadao = $this->input->get('nomeCidadao');
        $cpfCidadao = $this->input->get('cpfCidadao');
        $profissional = $this->input->get('profissional');
        $unidade = $this->input->get('unidade');
        $especialidade = $this->input->get('especialidade');
        $status = $this->input->get('status_da_solicitacao');
        $horario = $this->input->get('horario');

        // Filtros para busca
        $filtros = [];
        // Filtro de data: se só uma data, busca exatamente aquele dia; se duas, busca intervalo
        if ($dataInicial && $dataFinal && $dataInicial == $dataFinal) {
            $filtros['data_exata'] = $dataInicial;
        } else {
            if ($dataInicial) $filtros['data'] = $dataInicial;
            if ($dataFinal) $filtros['data2'] = $dataFinal;
        }
        if ($nomeCidadao) $filtros['pesquisa'] = $nomeCidadao;
        if ($cpfCidadao) $filtros['cpf'] = $cpfCidadao;
        if ($status) $filtros['status_da_solicitacao'] = $status;
        // Filtros adicionais podem ser implementados conforme necessidade

        // Ajuste no model: se existir 'data_exata', filtrar por data_solicitante = $dataInicial
        $agendamentos = $this->Relatorios_model->AgendamentosComFiltro($filtros);

        $data = [];
        $data['tipo_analise'] = $tipo_analise;
        $data['dataInicial'] = $dataInicial;
        $data['dataFinal'] = $dataFinal;
        $data['title'] = 'Relatório Analítico de Agendamentos';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);

        // Monta agrupamentos analíticos conforme o tipo
        switch ($tipo_analise) {
            case 'paciente':
                $data['agendamentos'] = $agendamentos;
                break;
            case 'profissional':
                $data['analise_profissionais'] = $this->Relatorios_model->analisePorProfissional($agendamentos);
                break;
            case 'unidade':
                $data['analise_unidades'] = $this->Relatorios_model->analisePorUnidade($agendamentos);
                break;
            case 'status':
                $data['analise_status'] = $this->Relatorios_model->analisePorStatus($agendamentos);
                break;
            case 'data':
                $data['analise_datas'] = $this->Relatorios_model->analisePorData($agendamentos);
                break;
            case 'especialidade':
                $data['analise_especialidades'] = $this->Relatorios_model->analisePorEspecialidade($agendamentos);
                break;
            default:
                $data['agendamentos'] = $agendamentos;
        }
        ...
        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirRelatorioAnalitico', $data, true);
        pdf_create($html, 'relatorio_analitico_' . date('d-m-Y'), true);
    }
}
