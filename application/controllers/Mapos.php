...
class Mapos extends MY_Controller
{
    ...
    {
        // Verificar se idUnidadeSolicitante está na sessão, se não estiver, buscar do banco
        if (empty($this->session->userdata('idUnidadeSolicitante')) && $this->session->userdata('id')) {
            $this->load->model('usuarios_model');
            $usuario_data = $this->usuarios_model->getById($this->session->userdata('id'));
            if ($usuario_data && isset($usuario_data->idUnidadeSolicitante)) {
                $this->session->set_userdata('idUnidadeSolicitante', $usuario_data->idUnidadeSolicitante);
            }
        }
        
        $this->data['agendamentos'] = $this->mapos_model->getAgendamentosSolicitados();
    ...
    public function pesquisar()
    {
        ...
        $this->data['profissionaisSaude'] = $data['results']['profissionaisSaude'];
        $this->data['cidadaos'] = $data['results']['cidadaos'];
        $this->data['unidade_solicitante'] = $data['results']['unidade_solicitante'];
        $this->data['unidade_executante'] = $data['results']['unidade_executante'];
    ...
    /**
     * Endpoint para retornar os agendamentos em formato JSON para o calendário do painel
     */
    public function apiAgendamentos()
    {
        ...
        $agendamentos = $this->Agendamentos_model->getAgendamentosParaCalendario();
        
        // Debug: Log dos agendamentos antes do filtro
        log_message('debug', 'Total de agendamentos antes do filtro: ' . count($agendamentos));
        foreach ($agendamentos as $ag) {
            log_message('debug', 'Agendamento ID: ' . $ag->idAgendamento . ', Status: ' . $ag->status_da_solicitacao . ', Unidade: ' . $ag->unid_solicitantes_id);
        }
        
        // Aplicar filtros de permissão
        $permissao_nome = $this->session->userdata('permissao_nome');
        $id_unidade_solicitada = $this->session->userdata('idUnidadeSolicitante');
        
        // Debug: Log das informações de sessão
        log_message('debug', 'Permissão: ' . $permissao_nome . ', ID Unidade: ' . $id_unidade_solicitada);
        
        $agendamentos_filtrados = [];
        
        foreach ($agendamentos as $ag) {
            // Se for Admin ou Administrador, mostra tudo
            if ($permissao_nome == 'Admin' || $permissao_nome == 'Administrador') {
                $agendamentos_filtrados[] = $ag;
            } else if ($permissao_nome == 'Profissional Solicitante') {
                // Verifica se a unidade solicitante do agendamento corresponde à unidade do usuário logado
                $unidade_agendamento = isset($ag->unid_solicitantes_id) ? $ag->unid_solicitantes_id : null;
                
                if ($unidade_agendamento && $id_unidade_solicitada == $unidade_agendamento) {
                    $agendamentos_filtrados[] = $ag;
                }
            }
            // Outros tipos de permissão não visualizam nada
        }
        
        // Debug: Log dos agendamentos após o filtro
        log_message('debug', 'Total de agendamentos após filtro: ' . count($agendamentos_filtrados));
