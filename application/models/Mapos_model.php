...
class Mapos_model extends CI_Model
{
    ...
    public function pesquisar($termo)
    {
        ...
        $this->db->or_like('cpf', $termo);
        $this->db->or_like('cns', $termo);
        $this->db->limit(15);
        $data['cidadaos'] = $this->db->get('cidadao')->result();

        // buscando Profissionais de saúde
        $this->db->like('nome', $termo);
        $this->db->or_like('conselho', $termo);
        $this->db->limit(15);
        $data['profissionaisSaude'] = $this->db->get('profissional_saude')->result();

        // buscando unidades solicitantes
        $this->db->like('nome', $termo);
        $this->db->or_like('endereco', $termo);
        $this->db->limit(15);
        $data['unidade_solicitante'] = $this->db->get('unidade_solicitante')->result();

        // buscando unidades Executantes
        $this->db->like('nome', $termo);
        $this->db->or_like('endereco', $termo);
        $this->db->limit(15);
        $data['unidade_executante'] = $this->db->get('unidade_executante')->result();
    ...
    {
        $this->db->select('agendamentos.*, cidadao.nome, cidadao.cpf, usuarios.nome as usuario_solicitante, unidade_solicitante.nome as unidade_solicitante');
        $this->db->from('agendamentos');
        $this->db->join('cidadao', 'cidadao.idCidadao = agendamentos.cidadaos_id');
        $this->db->join('usuarios', 'usuarios.idUsuarios = agendamentos.prof_solicitantes_id');
        $this->db->join('unidade_solicitante', 'unidade_solicitante.idUnidadeSolicitante = agendamentos.unid_solicitantes_id');
        $this->db->where('agendamentos.status_da_solicitacao', 'SOLICITADO');
        $this->db->limit(10);
        $query = $this->db->get();
